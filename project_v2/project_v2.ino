...
float f32_amp_factor = 0.01245f; //0.00372549f; //needs to change
int speed;
...
void loop() {
  ...
  */
  ...
  //f32_correction = (kp * ((float)i32_GX_sum * f32_GX_conversion)) + (ki * ((float)i32_AY_sum * f32_AY_conversion)); //PI controller
  f32_correction = (kp * ((float)i16_AY * f32_AY_conversion)) + (ki * ((float)i32_AY_sum * f32_AY_conversion)); //PI controller


  /*
  *Operation count
  * Float:       1 (+ 1 for float to int)
  * Store:       3
  * Comparison:  2
  * Integer:     1
  *
  * Total: 8
  */
  speed = (int)(f32_correction / f32_amp_factor); //+ 0.5 for rounding? cost extra float operation
  if (speed < 0) {
    useDir1 = true;
    speed = -speed;
  } else {
  ...
  * I2C:         13        = 13   long 
  * Bitwise:     4         = 4    short
  * Integer:     2 + 2 + 1 = 5    medium
  * Store:       2 + 3 + 3 = 8    short
  * Float:       7 + 2     = 9    long
  * Comparison:  2 + 1     = 3    short
  * Signal:      3         = 3    short
  * 
  */
  end_time = micros();
  ...
  /*
  Serial.print("\ti16_AY_sum: "); Serial.print(i32_AY_sum * f32_AY_conversion);
  Serial.print("\ti16_AY: "); Serial.print(i16_AY * f32_AY_conversion);
  Serial.print("\ti16_GX_sum: "); Serial.print(i32_GX_sum * f32_GX_conversion);
  Serial.print("\ti16_GX: "); Serial.print(i16_GX * f32_GX_conversion);
  Serial.print("\tf32_correction: "); Serial.print(f32_correction); 
